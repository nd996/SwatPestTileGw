...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      do k = 1, npmx
		...
		wpstaao(k,5) = wpstaao(k,5) * 1.e6
		wpstaao(k,6) = wpstaao(k,6) * 1.e6
        wpstaao(k,7) = wpstaao(k,7) * 1.e6
     ...
     &            wpstaao(k,5), wpstaao(k,6), wpstaao(k,7)
     ...
     &            wpstaao(k,5), wpstaao(k,6), wpstaao(k,7)
        ...
        do j = 1, nhru
          if (hrupest(j) == 1) then
            ...
            do l = 1, 4
              pfg = pfg + pst_lag(k,l,j) * hru_dafr(j) !HR pesticide in lag parameters
            end do
            pfgw = pfgw + (pst_shallst(k,j) + rootzpest(k,j)) * hru_dafr(j) !HR
            pfgwdeep = pfgwdeep + pst_deepst(k,j) * hru_dafr(j) !HR
          end if
        end do
        !! change units from kg/ha to mg/ha
        pfp = pfp * 1.e6
        ...
        pfgw = pfgw * 1.e6
        if (iscen == 1) then
        write (26,5100) pfp, pfg, pfgw, pfgwdeep !HR
        else if (isproj == 1) then
        write (19,5100) pfp, pfg, pfgw, pfgwdeep !HR
        endif
      end do

      return
 5000 format (/,'AVERAGE ANNUAL PESTICIDE SUMMARY DATA, PESTICIDE #',i3,
     &    ': ',a17,/,t20,'APPLIED = ',f15.4,' mg/ha',/,t20,'DECAYED = ',
     &    f15.4,' mg/ha',/,t20,                                         
     ...
     &    ' mg/ha',/,t20,'IN TILE FLOW ENTERING STREAM = ',f15.4,    
     &    ' mg/ha',/,t20,'IN SHALLOW GROUNDWATER FLOW ENTERING STREAM = ',f15.4,    
     &    ' mg/ha',/,t20,'IN DEEP GROUNDWATER FLOW ENTERING STREAM = ',f15.4,
     &    ' mg/ha')

 5100 format (/,t20,'FINAL AMOUNT OF PESTICIDE ON PLANT = ',f15.4,      
     &    ' mg/ha',/,t20,'FINAL AMOUNT OF PESTICIDE IN GROUND = ',f15.4,
     &    ' mg/ha',/,t20,'FINAL AMOUNT OF PESTICIDE IN SHALLOW AQ = ',f15.4,
     &    ' mg/ha',/,t20,'FINAL AMOUNT OF PESTICIDE IN DEEP AQ = ',f15.4,
     &   ' mg/ha')
