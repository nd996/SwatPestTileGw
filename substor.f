...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      if (hrupest(j) == 1) then
        do k = 1, npmx
          !HR Add pesticide decay to watershed summary variable.
          x1 = pst_lag(k,3,j)
          if (x1 < 1.e-6) x1 = 0.0
          !MFW, 3/3/12: Modified lagged pesticide to include decay in lag          
          xx = x1 * decay_s(npno(k))
          wshd_pstdg(k) = wshd_pstdg(k) + (x1 - xx) * hru_dafr(j)
          pst_lag(k,3,j) = xx + lat_pst(k)
          ! pst_lag(k,3,j) = pst_lag(k,3,j) + lat_pst(k)

          !HR add tile storage and pesticide decay to watershed summary variable.
          x1 = pst_lag(k,4,j)
          if (x1 < 1.e-6) x1 = 0.0
          xx = x1 * decay_s(npno(k))
          wshd_pstdg(k) = wshd_pstdg(k) + (x1 - xx) * hru_dafr(j)
          pst_lag(k,4,j) = xx + tile_pst(k)
        end do
      end if
      ...
      if (hrupest(j) == 1) then
        !write(*,*) lat_ttime(j), tile_ttime(j)
        do k = 1, npmx
          lat_pst(k) = pst_lag(k,3,j) * lat_ttime(j)
          tile_pst(k)= pst_lag(k,4,j) * tile_ttime(j)
        end do
      end if
      ...
      if (hrupest(j) == 1) then
        do k = 1, npmx
          pst_lag(k,3,j) = pst_lag(k,3,j) - lat_pst(k)
          pst_lag(k,4,j) = pst_lag(k,4,j) - tile_pst(k)
        end do
      end if
